...
// Global path separator - defaults to system preferred, but can be changed for testing
...
//   "C:\\Users\\name\\src\\**\\*.cpp" -> "C:\\Users\\name\\src"
inline std::string getBaseDirectoryToWatch(const std::string &pattern) {
    ...
    if (basePath.empty()) {
        basePath = std::string(1, path_separator);
    }
...
inline bool matchGlobPattern(const std::string &pattern, const std::string &filename, size_t p = 0, size_t f = 0) {
        ...
        switch (pattern[p]) {
            case '*':
                ...
                if (p + 1 < pattern.size() && pattern[p + 1] == '*') {
                    ...
                    if (p < pattern.size() && pattern[p] == path_separator) {
                        ++p;
                    }

                    // Try matching ** with zero or more characters
                    for (size_t i = f; i <= filename.size(); ++i) {
                        if (matchGlobPattern(pattern, filename, p, i)) {
                            return true;
                        }
                    }
                    return false;
                ...
                for (size_t match = f; match <= filename.size() && (match == f || filename[match - 1] != path_separator); ++match) {
                    if (matchGlobPattern(pattern, filename, p, match)) {
                        return true;
                    }
                }
                return false;
            ...
            case '{':
                // Shell brace pattern, find closing brace
                {
                    ...
                    for (size_t start = p + 1, i = start; i < close; ++i) {
                        if (pattern[i] != ',' && i != close - 1) {
                            continue;
                        }
                        size_t end = (pattern[i] == ',') ? i : close;
                        std::string alt = pattern.substr(0, p) + pattern.substr(start, end - start) + pattern.substr(close + 1);
                        if (matchGlobPattern(alt, filename, 0, 0)) {
                            return true;
                        }
                        start = i + 1;
                    }
